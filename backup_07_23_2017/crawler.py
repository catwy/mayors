        ...
        for table_title, table in result.iteritems():
            ...
            if camel_title not in ['LastGeneralElection', 'PrimaryOtherSchedule']:
                with open('data/{}_{}_{}.json'.format(description, uid, camel_title), 'wb') as fp:
                    json.dump(table, fp)
            if category == 'race' and 'Mayor' not in description:
                continue
...
if __name__ == '__main__':
    ...
    list = {1,2,3}
    for x in list:
        with open('recent_elections_part{}.txt'.format(x), 'rb') as fp:
            #with open('recent_elections_part1.txt', 'rb') as fp:
            reader = csv.reader(fp, delimiter=';')
            for row in reader:
                url, city, state, partisan = row[:4]
                if "http" in url:
                    url = url.strip()[28:]
                    city = city.strip()
                    state = state.strip()
                    partisan = partisan.strip()
